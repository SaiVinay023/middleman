...
export function AdaptiveNav() {
  ...
  // 1. Define Nav Logic based on Role
  const isAdmin = profile?.role === 'admin';

  const freelancerItems = [
    { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
    { name: 'Profile', href: '/dashboard/profile', icon: User },
    { name: 'Gigs', href: '/dashboard/gigs', icon: Search, isCenter: true },
  ...
  const navItems = isAdmin ? adminItems : freelancerItems;
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] font-black uppercase mt-2 transition-opacity duration-300 ${
                  isActive ? (isAdmin ? 'text-red-400' : 'text-blue-400') + ' opacity-100' : 'opacity-0'
                }`}>
                  {item.name}
                </span>
              </Link>
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center flex-1 h-16 transition-all duration-300 pb-2 ${
                isActive ? (isAdmin ? 'text-red-400' : 'text-blue-400') : 'text-gray-500'
              }`}
            >
              <div className="relative">
                <Icon size={isActive ? 24 : 22} className="transition-transform duration-300" />
                {isActive && (
                  <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full shadow-[0_0_8px] ${
                    isAdmin ? 'bg-red-400 shadow-red-400' : 'bg-blue-400 shadow-blue-400'
                  }`} />
                )}
              </div>
